...
$paymentList = [];
$totalRevenue = 0;
$isNoPayActive = true; // پیش‌فرض تنظیمات
...
try {
    ...
    // 2. دریافت وضعیت تنظیمات (برای مودال)
    $stmtSet = $pdoReport->prepare("SELECT setting_value FROM settings WHERE setting_key = 'enable_no_prepayment'");
    $stmtSet->execute();
...
?>
    ...
    <div class="container-fluid py-5 px-lg-5">
        ...
        <?php if (isset($_GET['status']) && $_GET['status'] == 'settings_updated'): ?>
            <div class="alert alert-success alert-dismissible fade show mb-4">
                ⚙️ تنظیمات سیستم پرداخت با موفقیت بروزرسانی شد.
        ...
        <div class="card">
            ...
            <div class="card-body p-0">
                <div class="tab-content">
                    ...
                    <!-- Tab: Payments (اصلاح شده با ستون‌های جدید) -->
                        ...
                        <div class="table-responsive">
                            <table class="table table-modern mb-0 w-100">
                                ...
                                <?php if(empty($paymentList)): ?>
                                    <tr><td colspan="7" class="text-center py-5 text-muted">هیچ واریزی برای این سمینار ثبت نشده است.</td></tr>
                                <?php else: ?>
                                    <?php foreach ($paymentList as $pay): ?>
                                        <tr>
                                            ...
                                            <td class="font-monospace text-muted small"><?= $pay['phone'] ?></td>
                                            <td class="fw-bold text-primary fs-6">
                                                <?= number_format($pay['amount']) ?>
                                            </td>
                                            <!-- نمایش روش پرداخت (جدید) -->
                                            <td>
                                                <?php
                                                $methods = [
                                                        'card_to_card' => '<span class="badge bg-info text-dark bg-opacity-25 border border-info">کارت به کارت</span>',
                                                        'pos' => '<span class="badge bg-secondary bg-opacity-25 text-dark border border-secondary">کارتخوان</span>',
                                                        'cash' => '<span class="badge bg-success bg-opacity-25 text-dark border border-success">نقدی</span>',
                                                        'no_prepayment' => '<span class="badge bg-warning text-dark border border-warning">بدون پرداخت</span>'
                                                ];
                                                echo $methods[$pay['payment_method']] ?? $pay['payment_method'];
                                                ?>
                                            </td>

                                            <td><span class="badge bg-light text-dark border"><?= htmlspecialchars($pay['registrar_expert'] ?? '-') ?></span></td>
                                            ...
                                            <td class="text-end pe-4">
                                                ...
                                                <!-- دکمه عکس دوم (جدید) -->
                                                <?php if(!empty($pay['receipt_image_2'])): ?>
                                                    <button type="button" class="btn btn-sm btn-outline-dark rounded-pill px-2 ms-1"
                                                            onclick="showReceipt('<?= BASE_URL ?>/public/uploads/receipts/<?= $pay['receipt_image_2'] ?>')">
                                                        ۲
                                                    </button>
                                                <?php endif; ?>

                                                <!-- نمایش توضیحات اگر بود (جدید) -->
                                                <?php if(!empty($pay['description'])): ?>
                                                    <button type="button" class="btn btn-sm btn-light border ms-1" data-bs-toggle="tooltip" title="<?= htmlspecialchars($pay['description']) ?>">
